...
object GitHooks extends AutoPlugin {
  ...
  object autoImport {
    ...
    val gitPrepareCommitMessageHook: SettingKey[Option[File]] = sbt.settingKey("file to use for prepare-commit-msg hook")
  ...
  private def overrideHooks(state: State): Unit = {
    ...
    overrideFileIfNeeded(baseHookDirectory, "pre-applypatch", gitPreApplyPatchHook.in(currentRef).get(structure.data).get)
    overrideFileIfNeeded(baseHookDirectory, "pre-commit", gitPreCommitHook.in(currentRef).get(structure.data).get)
    overrideFileIfNeeded(baseHookDirectory, "prepare-commit-msg", gitPrepareCommitMessageHook.in(currentRef).get(structure.data).get)
